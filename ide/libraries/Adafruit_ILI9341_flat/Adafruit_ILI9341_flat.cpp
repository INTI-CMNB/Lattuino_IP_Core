...
#define SPI_BEGIN()             if(USE_HARD_SPI){SPI_OBJECT.begin();}
#define SPI_BEGIN_TRANSACTION() if(USE_HARD_SPI){HSPI_BEGIN_TRANSACTION();}
#define SPI_END_TRANSACTION()   if(USE_HARD_SPI){HSPI_END_TRANSACTION();}
#define SPI_WRITE16(s)          if(USE_HARD_SPI){HSPI_WRITE16(s);}else{SSPI_WRITE16(s);}
#define SPI_WRITE_PIXELS(c,l)   if(USE_HARD_SPI){HSPI_WRITE_PIXELS(c,l);}else{SSPI_WRITE_PIXELS(c,l);}

static const uint8_t *Adafruit_ILI9341::builtInFont=0;
...
static const uint8_t PROGMEM initValues[]=
{
 ...
 0xFF,0xF7,0x20,
 0xFF,0xEA,0x00,0x00,
 0xFF,ILI9341_PWCTR1,0x23, // Power control VRH[5:0]
 0xFF,ILI9341_PWCTR2,0x10, // Power control SAP[2:0];BT[3:0]
 0xFF,ILI9341_VMCTR1,0x3E,0x28, // VCM control
 0xFF,ILI9341_VMCTR2,0x86,      // VCM control2
 0xFF,ILI9341_MADCTL,0x48,      // Memory Access Control
 0xFF,ILI9341_VSCRSADD,0x00,0x00, // Vertical scroll (Zero)
 0xFF,ILI9341_PIXFMT,0x55,
 0xFF,ILI9341_FRMCTR1,0x00,0x18,
 0xFF,ILI9341_DFUNCTR,0x08,0x82,0x27, // Display Function Control
 0xFF,0xF2,0x00, // 3Gamma Function Disable
 0xFF,ILI9341_GAMMASET,0x01, // Gamma curve selected
 0xFF,ILI9341_GMCTRP1,0x0F,0x31,0x2B,0x0C,0x0E,0x08,0x4E,0xF1,0x37,0x07,0x10,0x03,0x0E,0x09,0x00, // Set Gamma
 0xFF,ILI9341_GMCTRN1,0x00,0x0E,0x14,0x03,0x11,0x07,0x31,0xC1,0x48,0x08,0x0F,0x0C,0x31,0x36,0x0F  // Set Gamma
};

#ifdef ESP32
void Adafruit_ILI9341::begin(uint32_t freq, SPIClass &spi, const uint8_t *aFont)
#else
void Adafruit_ILI9341::begin(uint32_t freq, const uint8_t *aFont)
#endif
{
#ifdef ESP32
    _spi = spi;
    ...
    uint8_t i=0;
    while (i<sizeof(initValues))
      {
       uint8_t v=pgm_read_byte(initValues+i);
       i++;
       if (v==0xFF)
         {
          v=pgm_read_byte(initValues+i);
          i++;
          writeCommand(v);
         }
       else
...
void Adafruit_ILI9341::writeColor(uint16_t color, uint32_t len){
    ...
    if(USE_HARD_SPI){ //AVR Optimization
...
void Adafruit_ILI9341::fillRoundRect(int16_t x, int16_t y, int16_t w,
...
void Adafruit_ILI9341::writeLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1,
        uint16_t color) {
...
void Adafruit_ILI9341::drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1,
        uint16_t color) {
    if(x0 == x1){
...
void Adafruit_ILI9341::drawCircle(int16_t x0, int16_t y0, int16_t r,
        uint16_t color) {
    int16_t f = 1 - r;
    int16_t ddF_x = 1;
    int16_t ddF_y = -2 * r;
...
void Adafruit_ILI9341::drawCircleHelper( int16_t x0, int16_t y0,
...
void Adafruit_ILI9341::fillCircle(int16_t x0, int16_t y0, int16_t r,
        uint16_t color) {
    startWrite();
...
void Adafruit_ILI9341::fillCircleHelper(int16_t x0, int16_t y0, int16_t r,
...
// Draw a rectangle
void Adafruit_ILI9341::drawRect(int16_t x, int16_t y, int16_t w, int16_t h,
        uint16_t color) {
    startWrite();
    writeFastHLine(x, y, w, color);
...
// Draw a triangle
void Adafruit_ILI9341::drawTriangle(int16_t x0, int16_t y0,
...
// Fill a triangle
void Adafruit_ILI9341::fillTriangle(int16_t x0, int16_t y0,
        int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color) {
...
void Adafruit_ILI9341::drawRoundRect(int16_t x, int16_t y, int16_t w,
        int16_t h, int16_t r, uint16_t color) {
    // smarter version
    startWrite();
...
void Adafruit_ILI9341::drawXBitmap(int16_t x, int16_t y,
        const uint8_t *bitmap, int16_t w, int16_t h, uint16_t color) {
...
void Adafruit_ILI9341::drawChar(int16_t x, int16_t y, unsigned char c,
    ...
    if(USE_BUILT_IN_FONT) { // 'Classic' built-in font
        ...
        startWrite();
        int16_t xoff = x, yoff;
        const uint8_t *font = builtInFont+(c*5);
        for(int8_t i=0; i<6; i++, font++ ) {
            uint8_t line;
            if(i < 5) line = pgm_read_byte(font);
            else      line = 0x0;
            yoff = y;
            for(int8_t j=0; j<8; j++, line >>= 1) {
                if(line & 0x1) {
                    if(size == 1) writePixel(xoff, yoff, color);
                    else          writeFillRect(xoff, yoff, size, size, color);
                } else if(bg != color) {
                    if(size == 1) writePixel(xoff, yoff, bg);
                    else          writeFillRect(xoff, yoff, size, size, bg);
                }
                yoff += size;
            }
            xoff += size;
...
#if ARDUINO >= 100
 #define WRITE_RETURN size_t
#else
 #define WRITE_RETURN void
#endif

WRITE_RETURN Adafruit_ILI9341::write(uint8_t c) {
    if(USE_BUILT_IN_FONT) { // 'Classic' built-in font
...
void Adafruit_ILI9341::setCursor(int16_t x, int16_t y) {
    cursor_x = x;
    cursor_y = y;
}

int16_t Adafruit_ILI9341::getCursorX(void) const {
    return cursor_x;
}

int16_t Adafruit_ILI9341::getCursorY(void) const {
    return cursor_y;
}

void Adafruit_ILI9341::setTextSize(uint8_t s) {
    textsize = (s > 0) ? s : 1;
}

void Adafruit_ILI9341::setTextColor(uint16_t c) {
    // For 'transparent' background, we'll set the bg
    // to the same as fg instead of using a flag
    textcolor = textbgcolor = c;
}

void Adafruit_ILI9341::setTextColor(uint16_t c, uint16_t b) {
    textcolor   = c;
    textbgcolor = b;
}

void Adafruit_ILI9341::setTextWrap(boolean w) {
    wrap = w;
}

uint8_t Adafruit_ILI9341::getRotation(void) const {
    return rotation;
}
...
void Adafruit_ILI9341::cp437(boolean x) {
    _cp437 = x;
}

#if ENABLE_CUSTOM_FONT
void Adafruit_ILI9341::setFont(const GFXfont *f) {
...
#if !ENABLE_CUSTOM_FONT
void Adafruit_ILI9341::setFont(const GFXfont *) {
}
#endif


// Pass string and a cursor position, returns UL corner and W,H.
void Adafruit_ILI9341::getTextBounds(char *str, int16_t x, int16_t y,
...
void Adafruit_ILI9341::getTextBounds(const __FlashStringHelper *str,
...
int16_t Adafruit_ILI9341::width(void) const {
    return _width;
}

int16_t Adafruit_ILI9341::height(void) const {
    return _height;
}
...
GFXcanvas1::GFXcanvas1(uint16_t w, uint16_t h) : Adafruit_ILI9341(w, h) {
...
GFXcanvas16::GFXcanvas16(uint16_t w, uint16_t h) : Adafruit_ILI9341(w, h) {
...
void Adafruit_GFX_Button::initButton(
 Adafruit_ILI9341 *gfx, int16_t x, int16_t y, uint16_t w, uint16_t h,
...
void Adafruit_GFX_Button::initButtonUL(
 Adafruit_ILI9341 *gfx, int16_t x1, int16_t y1, uint16_t w, uint16_t h,
 uint16_t outline, uint16_t fill, uint16_t textcolor,
 char *label, uint8_t textsize)
{
  _x1           = x1;
